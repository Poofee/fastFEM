...
%     saveas(gcf,[num2str(count) '.jpg']);
    ...
    plot(ua,r(ua),'ok','MarkerSize',0.8,'markerfacecolor','black')
    else
    line([ua,ua],[ia,r(ua)],'linestyle',':','LineWidth',0.4,'Color','k')
    plot(ua,r(ua),'ok','MarkerSize',1.6,'markerfacecolor','black')
    end
    if textcount <10
...
%         saveas(gcf,[num2str(count) '.jpg']);
    count = count+1;
        ...
        if textcount > 10
        line([0,ua],[0,r(ua)],'Color','red','LineWidth',0.1,'LineStyle','--')
        else
            line([0,ua],[0,r(ua)],'Color','red','LineWidth',0.4,'LineStyle','--')
        end
%         pause(2)
%         saveas(gcf,[num2str(count) '.jpg']);
    count = count+1;
