...
% 注意：本程序起初为了便于调试，方便理解动态求解的过程，加快研究的进度，并不是采用正常
% 的动态特性求解过程。而是，在下一时刻，直接根据flux的结果，将衔铁移动到下一位置，这样，
% 就能严格的做到跟flux对比一致。只要电流波形正确，则代表算法正确，可以将电磁力的求解加入。
% 参考文献：
...
% time = [0,0.001,0.002,0.003,0.004,0.005,0.006,0.007,0.008,0.009,0.01,0.011,0.012,0.013,0.014,0.015,0.0151,0.0152,0.0153,0.0154,0.0155,0.0165,0.017,0.0171,0.0172,0.0173,0.0174,0.0175,0.0185,0.0195,0.02,0.021,0.022,0.023,0.024,0.025,0.026,0.027,0.028,0.029,0.03,0.031,0.032,0.033,0.034,0.035,0.04,0.045,0.05,0.06,0.07,0.08,0.09,0.1];
time = [0:1e-3:15e-3,...
...
position_flux = [0,-5.24642361495153e-06,-2.68101213531427e-05,-7.76591015679292e-05,-0.000170930856806491,-0.000319240672326960,-0.000534628907106191,-0.000828518596095625,-0.00121214520184128,-0.00169682271387370,-0.00229491542355361,-0.00302092126264144,-0.00389287031998499,-0.00493527738685913,-0.00618519707416504,-0.00631293055523800,-0.00644349837490657,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065,-0.0065];
cur_disp = 0;
Ak = 0;
Ak1 = 0;
FixNL = 0;
...
fluxresultnew(ax1,ax2,ax3,ax4,ax5,ax6);
t=2;
...
for t=2:length(time)
    ...
    if time(t) > 0.02
        ...
        pos = position_flux(t-1);
    end
    cmd = ['gmsh.exe -setnumber disp ', num2str(-pos),' -2 -format msh2 model.geo '];
